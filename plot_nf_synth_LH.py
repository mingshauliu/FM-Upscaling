"""Truth vs pred plot for NF run on FM-synthesized LH gas maps.

After running:  ./submit.sh infer
this script stacks the per-sample synthetic gas files, runs NF inference,
and plots nf_truth_vs_pred_LH (synthetic).
"""
...
NF_CKPT  = "/mnt/home/mliu1/pipeline_v3/nf_checkpoints/l25/nf-epoch=242-val_loss=0.0000.ckpt"
SYNTH_DIR = "/mnt/home/mliu1/ceph/CAMELS-L25n256/synth_v3/L25_LH"
PARAMS    = "/mnt/home/mliu1/ceph/CAMELS-L25n256/cached/param_IllustrisTNG_LH_L25n256_normed.txt"
N_POSTERIOR = 2000
OUT = "nf_truth_vs_pred_LH_synth.png"

# Stack per-sample .npy into a single array
files = sorted(glob.glob(os.path.join(SYNTH_DIR, "sample_*.npy")))
if not files:
    print(f"No synthetic samples found in {SYNTH_DIR}")
    print("Run  ./submit.sh infer  first.")
    sys.exit(1)

print(f"Stacking {len(files)} synthetic gas maps...")
gas_synth = np.stack([np.load(f) for f in files], axis=0)
print(f"  Shape: {gas_synth.shape}")

# Save stacked file for reuse
stacked_path = os.path.join(SYNTH_DIR, "gas_synth_stacked.npy")
np.save(stacked_path, gas_synth)
print(f"  Saved stacked: {stacked_path}")
...
ds = CosmoVolumeDataset(gas_path=stacked_path, param_path=PARAMS, num_cosmo=2, crop_size=None)
...
plt.suptitle("NF on FM Synthetic Gas (LH L25)", fontsize=14)
plt.tight_layout()
